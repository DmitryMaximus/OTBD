...
import time
from PyQt5.Qt import QMessageBox, QMenu,QFileDialog
...
class DataFrameModel(QtCore.QAbstractTableModel):
    ...
    def data(self, index, role=QtCore.Qt.DisplayRole):
        ...
        #    return QtWidgets.QColor(255,255,204)
...
class FilterListMenuWidget(QtWidgets.QWidgetAction):
    ...
    def _populate_list(self, inital=False):
        ...
        if inital:
            ...
            build_list = disp_col
        for i in sorted(set(map(str,build_list))):
            _build_item(i)
...
class DataFrameWidget(QtWidgets.QTableView):
    ...
    def _set_flag(self):
        ...
        index = self._data_model.df.index.to_list()
        self._data_model.reset()
        for element in index:
            self.df.at[element,'SSW_flag'] = 1
        self._data_model.setDataFrame(self.df)
    ...
    def make_cell_context_menu(self, menu, row_ix, col_ix):
        ...
        menu.addAction(self._icon('DialogApplyButton'),"Mark_selection", self._set_flag)
        menu.addAction(self._icon('BrowserStop'),"Unmark_selection", self._unset_flag)
        # Quick Filter
